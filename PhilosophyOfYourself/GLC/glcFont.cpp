...
void glcFont::Load(const string &s, int iSize) {
	...
	if (c_mpi == c_mpFont.end()) {
		throw glcException("angefragtes font (" + s + ") noch nicht geladen");
	}
	
	m_sPath = sPath;
	
	if (iSize < c_mpi->second.iSizeMin) {
		throw glcException("angefragtes groesse vom font (" + s + ") nicht vorhanden");
	}
	m_iSize = iSize;
}
...
void glcFont::RenderText(const std::string& text, const tbColor& amb, const tbColor& diff, const tbColor& spec,
						 const tbVector3 &v1, const tbVector3 &v2, const tbVector3 &v3, const tbVector3 &v4,
						 bool bDisableDepthTest, float fOffset) {
	
	glPushAttrib(GL_ALL_ATTRIB_BITS);
	
	glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT, amb.c);
	glMaterialfv(GL_FRONT_AND_BACK, GL_DIFFUSE, diff.c);
	glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, spec.c);
...
void glcFont::RenderText(const std::string& text, const glcMaterial& pMat,
						 const tbVector3 &v1, const tbVector3 &v2, const tbVector3 &v3, const tbVector3 &v4,
						 bool bDisableDepthTest, float fOffset) {
...
void glcFont::DeleteTTF	(string sPath, bool bCompleteThePath)	{
	if (bCompleteThePath)
		sPath = c_sPathStart + sPath + c_sPathEnd;
	
	c_mpi = c_mpFont.find(sPath);
	if (c_mpi == c_mpFont.end()) {
		throw glcException("TTF font to be deleted does not exist");
	} else {
		for (int i = 0; i < static_cast<int>(c_mpi->second.vFont.size()); i++) {
			if(c_mpi->second.vFont[i]) {
				TTF_CloseFont(c_mpi->second.vFont[i]);
				c_mpi->second.vFont[i] = NULL;
			}
		}
		c_mpi->second.vFont.clear();
		c_mpFont.erase(c_mpi);
	}
}
