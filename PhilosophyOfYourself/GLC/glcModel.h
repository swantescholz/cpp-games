...
struct S_ModelInternOneSurface {	//fuer flachen beim laden von OFF
	int iNumVertices;				//anzahl der ecken
	vector<tbVector3> vVertices;	//ecken
	tbVector3 vNormal;				//normalenvektor der surface
};
//lighting + texture2d jetzt ok
//am besten: glTexGeni(GL_S, GL_TEXTURE_GEN_MODE, GL_OBJECT_LINEAR);
//und      : glTexGenfv(GL_S, GL_EYE_PLANE, {1,0,0,0}(als array));
//und das Gleiche fuer GL_T!
...
public:
	...
	void SetScale(const tbVector3 &v)	{m_vScale  = v; Update();}		//standart-scalierung
	...
	//funktion(en) zum laden eines models aus einem file; [alle modelle sollten in die negative z-achse blicken; mit center auf zero; laenge auf z-achse=2!,r=1]
	//Bei Blender: in positive y-achse schauend, 2 einheiten lang auf der y-achse, positive z-achse oben, mittelpunkt auf zero, (r=1?) 
	...
	static void LoadOFF(string sPath, const float pfSmoothingAngle = 0.0f, const tbMatrix &mTransform = tbMatrixIdentity());	//kann models aus object file format (off) einlesen 
	
	
	static void DeleteOFF(string sPath, bool bCompleteThePath = true); //deletes one model
