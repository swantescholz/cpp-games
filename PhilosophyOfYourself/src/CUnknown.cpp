...
CUnknown::CUnknown(tbVector2 pvStartPos, string psThought)
{
	m_OT = OT_UNKNOWN;
	m_sThought = psThought;
	m_TexUnknown.Load("unknown");
...
void CUnknown::Update(list<S_LevelIntern> *palObjects) {
...
bool CUnknown::PushY(list<S_LevelIntern> *palObjects, CObject *pObj) {
...
bool CUnknown::PushX(list<S_LevelIntern> *palObjects, CObject *pObj) {
	if (pObj->Collision(m_vPos,m_vWH) == false) return true;
...
void CUnknown::AddThought(list<S_LevelIntern> *palObjects) {
	...
	tmpObj.pObj = new CThought(m_sThought, "FreeSans", iSize, glcMaterial(tbColor(0.07,0.15,0.17,1.0),tbColor(0.07,0.15,0.17,1.0),tbColor(0.0,0.0,0.0,1.0),20.0),
...
bool CUnknown::Collision(tbVector2 pvCenter, tbVector2 pvWH) {
