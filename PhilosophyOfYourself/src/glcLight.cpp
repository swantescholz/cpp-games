...
int glcLight::c_iNumUses[] = {0,0,0,0, 0,0,0,0};

glcLight::glcLight(const glcSphere &sphere, bool bActive, bool bPos, tbColor c1, tbColor c2, tbColor c3, float kc, float kl, float kq,
				   tbVector3 vDir, float fEx, float fCut)
	: m_Sphere(sphere)
{
	c_iCounter++;
	SetActive(bActive);
	...
	for (int i = 0; i < GLC_MAXLIGHTS; i++) {
		if (c_bLights[i] == false) {
			m_iLight = i;
			c_bLights[i] = true;
			break;
		}
	}
	if (m_iLight == -1) throw glcException("too many lights");
	...
	else g_pLogfile->Textout(BLUE,"warning: new light already enabled");
	
	//c_bPositional[m_iLight] = bPos;
	c_iNumUses[m_iLight]++;
}

glcLight::glcLight(const glcLight &rhs) : m_Sphere(rhs.m_Sphere) {
	c_iCounter++;
	SetActive(rhs.m_bActive);
	SetPositional(rhs.m_bPositional);
	SetMaterial(rhs.m_mat.ambient,rhs.m_mat.diffuse,rhs.m_mat.specular);
	SetAttentuation(rhs.m_fkc,rhs.m_fkl,rhs.m_fkq);
	SetSpot(rhs.m_vSpotDirection,rhs.m_fSpotExponent,rhs.m_fSpotCutoff);
...
void glcLight::Render() {
	if (m_bActive && glIsEnabled(GL_LIGHTING)) {
		...
		glDisable(GL_LIGHTING);
			...
			m_Sphere.Render();					//comment this out to hide the light_sphere
