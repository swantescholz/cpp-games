...
//glc - flags fuer init
#define GLC_LIGHTING 	nflag::a8[0]	//lighting, not shadowing
#define GLC_BLEND 		nflag::a8[1]	//alpha-blending
#define GLC_CULLING 	nflag::a8[2]	//clock-wise surfaces invisible
#define GLC_FOG		 	nflag::a8[3]	//simple fog
#define GLC_NONE		nflag::a8[4]	//nothing
#define GLC_TEXTURE2D	nflag::a8[5]	//2D textures enabled
...
namespace nmath { // praktische kleine (mathematische) funktionen
	...
	template<class T> inline T Clamp(T I, T A, T B) {if (I < A) I=A; if(I>B) I=B; return I;}
	//double Clamp(double p,double min = 0.0,double max =1.0) {if (p<min) p = min; if (p>max) p = max; return p;}
...
public:
	...
	//eingabe
	bool KeyIsDown (int key_ID) const		{return m_aKeyIsDown[key_ID];}
	...
	void SetKeyWasDown(int key_ID,bool bState)		{m_aKeyWasDown[key_ID] = bState;}
	void SetKeyWasUp  (int key_ID,bool bState)		{m_aKeyWasUp[key_ID]   = bState;}
	int  GetMouseXPosi () const {return m_iMouseXPos;}
	int  GetMouseYPosi () const {return m_iMouseYPos;}
	float GetMouseXPosf () const {return m_fMouseXPos;}
	float GetMouseYPosf () const {return m_fMouseYPos;}
	bool MouseIsDown(int key_ID) const	{return m_aMouseIsDown[key_ID];}
	...
	void ResetAllEvents();	//resets all     events of mouse and keyboard
	void ResetIsEvents();	//resets all  is-events of mouse and keyboard
	void ResetWasEvents();	//resets all was-events of mouse and keyboard
	
	//window
	int GetAllW() const {return m_iAllW;}
	int GetAllH() const {return m_iAllH;}
