...
/**SDL_ttf**/
	...
	if (TTF_Init() != 0) {
		throw glcException(string("TTF_Init() failed: ") + TTF_GetError());
	}

/**SDL_image**/

/**SDL_mixer**/
	//nur bei music: 4096; ansonsten 1024 oder kleiner	(std: 512) (bei niedrigen werten ggf. bug bei mix_close_audio()-1) TODO
	if (Mix_OpenAudio(MIX_DEFAULT_FREQUENCY, MIX_DEFAULT_FORMAT, 2, 1024) != 0) {
		throw glcException("Mix_OpenAudio() failed");
	} 
...
/** OpenGL **/
	...
	if (GLflags & GLC_TEXTURE2D) {
		...
		glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE); //vorher: decal, std: modulate
...
void glcSDL_GL::Quit() {
	Mix_CloseAudio();	//<- exakt hier ggf. absturz bei geringer chunksize
	TTF_Quit();
	SDL_Quit();
}
...
void glcSDL_GL::ResetAllEvents() {
	for (int i = 0; i < SDLK_LAST; i++) {
		m_aKeyIsDown[i]  = false;
		m_aKeyWasDown[i] = false;
		m_aKeyWasUp[i]	 = false;
	}
	for (int i = 0; i < 10; i++) {
		m_aMouseIsDown[i]  = false;
		m_aMouseWasDown[i] = false;
		m_aMouseWasUp[i]   = false;
	}
}

void glcSDL_GL::ResetIsEvents() {
	for (int i = 0; i < SDLK_LAST; i++) {
		m_aKeyIsDown[i]  = false;
	}
	for (int i = 0; i < 10; i++) {
